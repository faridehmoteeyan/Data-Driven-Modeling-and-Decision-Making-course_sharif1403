...
cat("AIC_stepwisemodel =", AIC_stepwisemodel, "\n")
...
# محاسبه معیارها
TP <- T2[2, 2]  # True Positives
TN <- T2[1, 1]  # True Negatives
FP <- T2[1, 2]  # False Positives
FN <- T2[2, 1]  # False Negatives

# Accuracy
accuracy_test_stepwisemodel <- (TP + TN) / sum(T2)

# Sensitivity (True Positive Rate)
sensitivity_test_stepwise <- TP / (TP + FN)

# Specificity (True Negative Rate)
specificity_test_stepwise <- TN / (TN + FP)
...
T3 <- table(train$ANSWERED, predict_train_logistic > 0.9)


TP3 <- T3[2, 2]  # True Positives
TN3 <- T3[1, 1]  # True Negatives
FP3 <- T3[1, 2]  # False Positives
FN3 <- T3[2, 1]  # False Negatives

# Accuracy
accuracy_train_stepwisemodel3 <- (TP3 + TN3) / sum(T3)

# Sensitivity (True Positive Rate)
sensitivity_train_stepwise3 <- TP3 / (TP3 + FN3)

# Specificity (True Negative Rate)
specificity_train_stepwise3 <- TN3 / (TN3 + FP3)

# نمایش مقادیر
cat("sensitivity_train_stepwise3 =", sensitivity_test_stepwise3, "\n")
cat("specificity_train_stepwise3 =", specificity_test_stepwise3, "\n")
cat("accuracy_train_stepwisemodel3 =", accuracy_test_stepwisemodel3, "\n")

#-------------------------question 3 in test data-------------------------------

T4 <- table(test$ANSWERED, predict_test_logistic > 0.9)
...
# Accuracy
accuracy_test_stepwisemodel4 <- (TP4 + TN4) / sum(T4)

# Sensitivity (True Positive Rate)
sensitivity_test_stepwise4 <- TP4 / (TP4 + FN4)

# Specificity (True Negative Rate)
specificity_test_stepwise4 <- TN4 / (TN4 + FP4)
